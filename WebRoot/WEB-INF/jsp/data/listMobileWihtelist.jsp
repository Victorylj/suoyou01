...
function upqryMobileUserWihte(id){
	/* if(id>0){
		art.dialog.load('upqryMobileUserWihte.do?id='+id, {title: '绿名单',lock:true}, false);
	}else{
		art.dialog.load('upqryMobileUserWihte.do', {title: '绿名单',lock:true}, false);
	} */
	 $.ajax({  
        type : "POST",  //提交方式  
        url : "upqryMobileUserWihte.do",//路径  
        data : {  
            id:id
        },//数据，这里使用的是Json格式进行传输  
	...
	function getPageId(pageNum,nextPageId){
		...
		window.location = 'qryMobileUserWihte.do?pageId='+nextPageId+'&'+params;
	...
	//删除
	function delWihtelistMobile(id){
		art.dialog.confirm("确认删除该选项？", 
			function() {
				...
				var url = "delMobileUserWihte.do";
				var data = {"id" : id};
				$.post(url, data, function(result) {
					pop_succeed("温馨提示", result.msg, function() {
						if (result.errorCode == 0) {
							window.location ="qryMobileUserWihte.do";
						}
					}, false);
				}, "json");
			}
...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			<ul class="seachform">
			<form id="searcherForm" action="qryMobileUserWihte.do" method="post"  onsubmit="initshow()">
...
<ul class="toolbar">
					...
					<c:when test="${not empty page.list}">
						<c:forEach items="${page.list}" var="v" varStatus="var">
							<tr>
								...
								<td>${v.create_time}</td>
								<td>
								 <%-- <a href="upqryMobileUserWihte.do?id=${v.id}" class="tablelink"> 修改</a> --%>
								 <a href="javascript:upqryMobileUserWihte('${v.id}')" class="tablelink"> 修改</a>
								 <a href="javascript:delWihtelistMobile('${v.id}')" class="tablelink"> 删除</a>
								 </td> 
							</tr>
						</c:forEach>
					</c:when>
	...
	<div id="editStatus" style="display:none;">
	    <div class="formtitle"><span>白名单用户</span><font style="color:red;margin-left:120px;"></font></div>
	<div class="form_body">
				<form action="updMobileUserWihte.do" method="post">
					<ul class="form_info">
						<li><input name="id" id="id" value="" type="hidden" ></input>
						<label>用户号码</label>   <input class="dfinput" type="text" id="mobile" name="mobile"/>
						</li>
						
						<li><label>用户imsi</label>   <input class="dfinput" type="text" id="imsi" name="imsi"/>
						</li>
						<li></li>
