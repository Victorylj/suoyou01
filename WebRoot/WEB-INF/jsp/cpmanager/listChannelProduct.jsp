...
<script type="text/javascript" src="js/cpmanager/listChannelProduct.js"></script>
...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			<ul class="seachform">
				<form id="searcherForm" action="listChannelProduct.do" method="post" onsubmit="initshow()">
						...
						<div class="vocation">
						 ...
						 <select id="ch_id" name="ch_id" class="select3">
                		   <option value="">请选择</option>
                			 ...
                			 <option value="${ch.ch_id}" <c:if test="${searchObj.ch_id eq ch.ch_id}">selected="selected"</c:if> >${ch.ch_name}</option>
                		    </c:forEach>
                	   </select>
						...
						<div class="vocation">
						 ...
						 <select id="product_id" name="product_id" class="select3">
                		   <option value="">请选择</option>
                			 ...
                			 <option value="${pro.product_id}" <c:if test="${searchObj.product_id eq pro.product_id}">selected="selected"</c:if> >${pro.product_name}</option>
                		    </c:forEach>
                	   </select>
				...
				<c:choose>
					<c:when test="${not empty page.list}">
						<c:forEach items="${page.list}" var="v" varStatus="var">
							...
							<tr>
								<td><input type="checkbox" name="ch_prod_id" value="${ v.ch_prod_id}" /></td>
								<td>${v.ch_name }</td>
								<td>${v.ch_id }</td>
								...
								<td>${v.cp_id }</td>
								<td><a
									href="editChannelProduct.do?ch_prod_id=${v.ch_prod_id }"
									class="tablelink">修改</a> <a href="javascript:void()"
									onclick="deleteChannelProduct(${ v.ch_prod_id })" class="tablelink">
										删除</a></td>
							</tr>
