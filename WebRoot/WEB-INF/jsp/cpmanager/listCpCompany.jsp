...
<script type="text/javascript" src="js/cpmanager/listCpCompany.js"></script>
...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			<ul class="seachform">
				<form id="searcherForm" action="listCpCompany.do" method="post" onsubmit="initshow()">
					
					<li><label>渠道名称</label><input name="cp_name" type="text"
						class="scinput" /></li>
					<li><label>渠道编号</label><input name="cp_id" type="text"
						class="scinput" /></li>
				...
				<c:choose>
					<c:when test="${not empty page.list}">
						<c:forEach items="${page.list}" var="v" varStatus="var">
							...
							<tr>
								<td><input type="checkbox" name="cp_id" value="${ v.cp_id}" /></td>
								<td>${v.cp_name }</td>
								<td>${v.cp_id }</td>
								<%-- <td>${v.cp_ip }</td>
								<td>${v.mo_url }</td> --%>
								...
								<a href="editCpCompany.do?cp_id=${v.cp_id }" class="tablelink">修改</a>
								 <a href="javascript:void()" onclick="deleteCpCompany(${ v.cp_id})" class="tablelink"> 删除</a></td>
							</tr>
