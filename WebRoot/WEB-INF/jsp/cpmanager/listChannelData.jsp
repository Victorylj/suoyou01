				...
				<form id="searcherForm" action="listChannelData.do" method="post" onsubmit="initshow()">
					<li><label>渠道</label>
						<div class="vocation">
							...
							<select id="ch_id" name="ch_id" class="select3">
								<option value="">请选择</option>
								<c:forEach items="${chList}" var="ch">
									<option value="${ch.ch_id}"
										<c:if test="${searchObj.ch_id eq ch.ch_id}">selected="selected"</c:if>>${ch.ch_name}(${ch.ch_id})</option>
								</c:forEach>
							</select>
						</div></li>
					...
					<c:when test="${not empty page.list}">
						<c:forEach items="${page.list}" var="v" varStatus="var">
							...
							<tr>
								...
								<td>${v.dater }</td>
								<td><a
									href="editChannelData.do?ch_data_id=${v.ch_data_id }"
									class="tablelink">修改</a> <a href="javascript:void()"
									onclick="deleteChannelData(${ v.ch_data_id})" class="tablelink">
										删除</a></td>
							</tr>
