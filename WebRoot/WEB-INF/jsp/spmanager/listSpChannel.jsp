...
<script type="text/javascript" src="js/spmanager/listSpChannel.js"></script>
...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			<ul class="seachform">
				<form id="searcherForm" action="listSpChannel.do" method="post" onsubmit="initshow()">
					...
					<li><label>上游公司频道</label><input name="sp_channel_name" type="text"
						class="scinput" /></li>
				...
				<c:choose>
					<c:when test="${not empty page.list}">
						<c:forEach items="${page.list}" var="v" varStatus="var">
							...
							<tr>
								<td><input type="checkbox" name="sp_id" value="${ v.sp_channel_id}" /></td>
								<td>${v.sp_name }</td>
								<td>${v.sp_channel_name }</td>
								<td>${v.sp_channel_ip }</td>
								<td>${v.sp_channel_mo_url }</td>
								<td>${v.sp_channel_mr_url }</td>
								<td>
								...
								<a href="editSpChannel.do?sp_channel_id=${v.sp_channel_id }" class="tablelink">修改</a>
								 <a href="javascript:void()" onclick="deleteSpChannel(${ v.sp_channel_id})" class="tablelink"> 删除</a></td>
							</tr>
