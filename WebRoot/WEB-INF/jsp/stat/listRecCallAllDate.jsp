			...
			<ul class="seachform">
				<form id="searcherForm" action="listRecCallAllDates.do" method="post" onsubmit="initshow()">	
					...
					<li>
						...
						<li>
					<label>
						<input name="isop" type="radio" value="4" <c:if test="${searchObj.isop eq 4}">checked="checked" </c:if> />
						分运营商显示</label>
						<label>
						<input name="isop" type="radio" value="1" <c:if test="${searchObj.isop eq 1}">checked="checked" </c:if> />
						代码明细显示</label>
...
<c:if test="${searchObj.isop eq 4}">	
	<div class="divtable" style="overflow-x:hidden;">
		<table class="tablelist" >
			<thead><tr><th>上游公司</th><th>产品</th>
			<!-- <th>移动</th><th>联通</th><th>电信</th> -->
			<th>初始值</th>
			<th>非13金额</th>
			<!-- <th>移动</th><th>联通</th><th>电信</th> -->
			<th>上行条数</th>
			<!-- <th>移动</th><th>联通</th><th>电信</th> -->
			<th>成功条数</th>
			<th style="background:#BCD2EE">移动</th><th style="background:#BCD2EE">联通</th><th style="background:#BCD2EE">电信</th><th style="background:#BCD2EE">报盘</th>
			<th style="background:#8E8E38">移动</th><th style="background:#8E8E38">联通</th><th style="background:#8E8E38">电信</th><th style="background:#8E8E38">收入</th>
			<th>日期</th></tr></thead>
		</table>
	 </div>
		
	<div class="divtable">
		<table id="treetab1" class="tablelist" style="margin-top:-36px;">
			<thead>
				<tr>
				<th>上游公司</th>
				<th>产品</th>
				...
				<c:choose>
					<c:when test="${not empty page.list}">
					  ...
					  <c:set var="succ_numsum" value="0"></c:set>
					   ...
					   <c:set var="yd_feesum" value="0"></c:set>
					   <c:set var="lt_feesum" value="0"></c:set>
					   <c:set var="dx_feesum" value="0"></c:set>
					  <c:set var="ratessum" value="0"></c:set>
					  <c:set var="yd_ratessum" value="0"></c:set>
					  <c:set var="lt_ratessum" value="0"></c:set>
					  <c:set var="dx_ratessum" value="0"></c:set>
					  <c:set var="ptotal" value="0"></c:set>
						...
						<c:if test="${v.status==searchObj.status}">
							...
							<%-- 	<td>${v.yd_succ_calls}</td>
								<td>${v.lt_succ_calls}</td>
								<td>${v.dx_succ_calls}</td> --%>
								<td>${v.succ_calls}</td>
							<%-- 	<td>${v.yd_succ_num}</td>
								<td>${v.lt_succ_num}</td>
								<td>${v.dx_succ_num}</td> --%>
								...
								<c:set var="yd_ratessum" value="${yd_ratessum+v.yd_rates}"></c:set>	
								<c:set var="lt_ratessum" value="${lt_ratessum+v.lt_rates}"></c:set>	
								<c:set var="dx_ratessum" value="${dx_ratessum+v.dx_rates}"></c:set>		
								<c:set var="feesum" value="${feesum+v.fee}"></c:set>
								<c:set var="yd_feesum" value="${yd_feesum+v.yd_fee}"></c:set>
								<c:set var="lt_feesum" value="${lt_feesum+v.lt_fee}"></c:set>
								<c:set var="dx_feesum" value="${dx_feesum+v.dx_fee}"></c:set>
			...
			<tr><td colspan="4">总计</td><td>${succ_callsum}</td><td>${succ_numsum}</td>
			<td><fmt:formatNumber type="number" value="${yd_feesum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${lt_feesum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${dx_feesum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${feesum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${yd_ratessum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${lt_ratessum}" maxFractionDigits="2"/></td>
			<td><fmt:formatNumber type="number" value="${dx_ratessum}" maxFractionDigits="2"/></td>
			<td ><fmt:formatNumber type="number" value="${ratessum}" maxFractionDigits="2"/></td><td></td></tr>
			</tbody>
		</table>
	</div>
</c:if>

<c:if test="${ searchObj.isop != 4}">	
	<div class="divtable" style="overflow-x:hidden;">
		<table class="tablelist" >
