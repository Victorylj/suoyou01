...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
		<form id="searcherForm" action="listALLChannelStat.do" autocomplete="off" method="post" onsubmit="initshow()">
			<ul class="seachform">
				<li><label>渠道</label>
					...
					<c:when test="${not empty page.list}">
					 <c:set var="initnumsum" value="0"></c:set>
					  <c:set var="succ_userssum" value="0"></c:set>
					   ...
					   <c:set var="ofeesum" value="0"></c:set>
						<c:forEach items="${page.list}" var="v" varStatus="var">

							<tr id="${v.dater }">
								<td>${v.dater }</td>
								<td id="${v.cpid}">${v.cpname }</td>
								<td id="${v.proid}">${v.proid}</td>	
								<td id="${v.proid}">${v.proname }</td>			
								<td>${v.initnum }</td>
								<td>${v.succ_users }</td>
								<td onclick="edits(this,'fee')">${v.fee }</td>
								<td onclick="edits(this,'ofee')">${v.ofee }</td>
							</tr>
							...
							<c:set var="ofeesum" value="${ofeesum+v.ofee}"></c:set>	

						</c:forEach>
						<tr><td colspan='4'>总计</td><td>${initnumsum}</td><td>${succ_userssum}</td><td id="sumfee">${feesum}</td><td>${ofeesum}</td></tr>
					</c:when>
					<c:otherwise>
						<tr>
