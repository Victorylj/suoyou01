$(document).ready(function() {
		$(".click").click(function() {
			//$(".editdiv").fadeIn(200);
			window.location ="editSpInterfaceParam.do";
		});
	...
	function deleteSpInterfaceParam(ids){
		art.dialog.confirm("确认删除该接口配置？", 
			function() {
				if (!ids || ids == 0) {
					pop_warning("温馨提示", "请求参数有误", false);
					return;
				}
				var url = "deleteSpInterfaceParam.do";
				var data = {"arrayIds" : ids};
				$.post(url, data, function(result) {
				//	alert("dsfdsfda");
					pop_succeed("温馨提示", result.msg, function() {
						if (result.errorCode == 0) {
							window.location ="listSpInterfaceParam.do";
						}
					}, false);
				}, "json");
			}, 
